...
class VisitingEvent with _$VisitingEvent {
  /// Загрузить места
  const factory VisitingEvent.loadSights({@Default(false) bool hideLoading}) =
      _LoadSights;

  /// Удалить из посещённых
  const factory VisitingEvent.deleteFromVisited({required Sight sight}) =
      _DeleteFromVisited;
  ...
  const factory VisitingEvent.addWantToVisitTime(
      {required Sight sight, required DateTime date}) = _AddWantToVisitTime;

  /// Добавить место в favorite
  const factory VisitingEvent.addToWantToVisit({required Sight sight}) =
      _AddToWantToVisit;

  /// Добавить место в посещённые места
  const factory VisitingEvent.addToVisited({required Sight sight}) = _AddToVisited;
}
